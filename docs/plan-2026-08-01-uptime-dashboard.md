...
- **`UPTIMEROBOT_API_KEY` is live** in `.env.local` on mini, from 1Password `op://dev-secrets/UptimeRobot/api-key`, **and in Vercel Production** as of 2026-07-31 (this line previously said it was missing and went stale the same day it was written — Nico added it hours later). Nothing owed.
