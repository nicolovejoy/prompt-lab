...
5. §2.3 cleanup after verify — **DONE 2026-07-21, "Phase A" PR**: `beacon.py`'s `BEACON_SALT`→`AUTH_SECRET` fallback removed (unset `BEACON_SALT` now fails closed — the hit is dropped, endpoint stays opaque-204). Also fixed issue #30 in the same PR (preview's Google button was pinned to prod's redirect URI): the `GET /api/login` 401 body now also carries `google_login` (`VERCEL_ENV == "production"`), and `index.html`'s Login component renders the Google button only when it's true. **Nico's manual steps (not done by the agent):** delete `AUTH_READ_SECRET` from Vercel (verified: only `login.py:23` reads it — **note this also kills preview *reader* login; previews become admin-password-only**, consistent with decision 1); **set Preview/Dev `BEACON_SALT`** (still unset as of the PR — see CLAUDE.md §2.0 note). `AUTH_SECRET` is **demoted, not retired**: post-cutover it remains the HMAC token-signing key everywhere and the break-glass password in preview. Rotating it invalidates all cookies and in-flight `state` tokens.
6. §2.4 docs sweep — **DONE 2026-07-21, same PR**: `.env.tpl`, `README.md:85`, `docs/data-and-access.md:37,42`, roadmap STATE OF PLAY. (`CLAUDE.md` intentionally not touched — owned by the main session.)
