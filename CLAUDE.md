...
**Reconcile — concurrent Vercel Web Analytics tag (2026-07-05, `ad557d5`):** a parallel session (Opus 4.8) added `<script defer src="/_vercel/insights/script.js">` to `web/index.html` right beside `beacon.js`, so prompt-labs.org now loads **both** analytics. Not a conflict (additive), and arguably complementary — Vercel's tag gives the Vercel-native dashboard view (free on Hobby ≤50k/mo), while the beacon feeds our own `#/visitors` page (the export path that's Pro-gated is why we chose the beacon). **Decision pending:** keep both, or drop the Vercel tag to hold the clean first-party/no-third-party story. Slightly muddies the privacy narrative and double-beacons each load.
