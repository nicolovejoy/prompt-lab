...
### Cross-site visitor visibility (scoped 2026-07-05, issues #9 + #10)
Traced a 2026-06-14 ibuild4you ask ("visibility to who uses this app and all my cloudflare hosted domains") that never got filed — filed now, split in two: **#9** public-site traffic, **#10** auth-gated tool usage (tied to the existing OAuth-migration item below). DNS-audited every domain across `~/src` project memory: only 2 of ~14 are actually Cloudflare-proxied (musicforge.app, recordings.pianohouseproject.org) — everything else (11 domains incl. prompt-labs.org, ibuild4you.com, pianohouseproject.org) is Vercel-hosted with Cloudflare as DNS-only. Chose **Vercel Web Analytics + Drains** (option A) over a custom Turso-writing beacon (option B) for the MVP — full plan + step list is on issue #9, written up for Fable to review next session. **Open risk, must confirm before building:** whether Drains (the mechanism to export analytics data out of Vercel) requires a Pro/Enterprise plan upgrade — Nico explicitly doesn't want to pay more; if it's gated, fall back to option B.
