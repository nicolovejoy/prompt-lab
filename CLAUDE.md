...
Top-nav "Todos" link → `#/todos`. `web/api/todos.py` does one authenticated GitHub Search call (`is:open is:issue user:<GITHUB_USER>`, default nicolovejoy) for every open issue across **owned** repos, groups by repo (folded through the project alias map), renders per-repo. **Live-read — no table, no sync, always fresh.** Prominent total + project count; each repo is a **collapsed-by-default accordion** with Expand/Collapse-all. Needs `GITHUB_TOKEN` (read-only fine-grained PAT, Issues+Metadata) in Vercel env (Prod+Preview) + `.env.tpl` (`op://dev-secrets/prompt-lab-github-pat`); optional `GITHUB_USER`. **Open follow-ups (offered, not done):** (1) filter to dashboard-tracked projects only (currently lists every owned repo — SPAN, home-assistant, recountly, bakerylouise-v1…); (2) label/repo search box. **Caveats:** only repos you own (org/other-owned repos' issues won't appear); the PAT's expiry silently 401s the page when it lapses — regenerate longer-lived if it breaks.
