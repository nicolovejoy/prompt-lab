...
2. **freevite.app (invitekit's public domain) serves a DIFFERENT app — a Vite SPA, not the `invitekit` Next repo I instrumented.** The beacon IS live but only on `freevite.vercel.app` (records site=`freevite.vercel.app`, ugly). Real visitors hit freevite.app and get NO beacon. **Nico: which repo actually serves freevite.app?** The beacon must go there, not (only) invitekit. (invitekit repo has `src/app/layout.jsx` Next app-router but its live custom domain is a Vite build — the repo/domain wiring is confused.)
