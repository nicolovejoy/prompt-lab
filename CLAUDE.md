...
### /handoff writes to public_* tables — contradicts the safe-by-construction invariant (FLAGGED 2026-06-06)
The `/handoff` command's "public session summary" + "public weekly rollup" steps write to `public_session_summaries` / `public_weekly_rollups` programmatically. This **contradicts the invariant** documented above (`web/api/public_history.py` section): those tables are meant to be written ONLY by the hand-authored, scrubbed `scripts/backfill_public_*.py` — never by the synthesizer or handoff. The auto-approver blocked the handoff write on these grounds this session. Decide: either (a) remove the public-summary steps from `/handoff` (rely on backfill scripts as the single writer), or (b) relax the invariant to permit handoff's scrubbed writes and update the CLAUDE.md note. Until resolved, handoff's public-write steps will keep getting blocked.
