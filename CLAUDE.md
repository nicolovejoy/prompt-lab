...
So beacon now on **7 sites** incl. prompt-labs.org. **Still held:** **prntd** + **musicforge** had dirty working trees at fan-out time (do when clean; musicforge is Vite `frontend/src/main.tsx`, different injection than the Next repos). **Verify wave (pending):** load each live site in a real browser, confirm an event lands in Turso `page_views` with the correct Origin-derived `site` (browsers send Origin on cross-origin sendBeacon; verified via curl on prompt-labs.org but not yet a real cross-origin browser hit). **Local cleanup:** the sub-agents left byside/recountly/invitekit/selected-projects checked out on the deleted `add-visitor-beacon` branch — `git checkout main && git pull` in each before local work. Cloudflare-proxied musicforge.app + recordings.pianohouseproject.org and the unclear domains (eaglerockventures, robotorchestra, ruhuman) still covered identically once instrumented.
