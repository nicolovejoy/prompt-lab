...
**Step 2 fan-out — PARTIALLY UNBLOCKED 2026-07-05.** Nico said go. Opened one beacon PR per clean own-product repo (Sonnet sub-agents, `<Script src="https://prompt-labs.org/beacon.js" strategy="afterInteractive"/>` beside the existing `<Analytics/>` in each root layout — PRs, not pushes, so nothing hits main until reviewed):
- byside https://github.com/nicolovejoy/byside/pull/88
- ibuild4you https://github.com/nicolovejoy/ibuild4you/pull/118
- recountly https://github.com/nicolovejoy/recountly/pull/13
- invitekit https://github.com/nicolovejoy/invitekit/pull/77
- selected-projects https://github.com/nicolovejoy/selected-projects/pull/6 (root layout; no Vercel Analytics there — beacon is its only tracker; handoff note posted)

**Still held:** **prntd** + **musicforge** had dirty working trees at fan-out time (collision risk — do when clean; musicforge is Vite `frontend/src/main.tsx`, different injection than the Next repos). **bakerylouise-v1** is a CLIENT site — the beacon pipes visitor data into Nico's personal Turso dashboard, so mixing a client's traffic in needs Nico's explicit OK (Vercel Analytics there is fine — it's the site's own hosting analytics; the beacon is the propriety question). **After merge:** verify wave — load each live site, confirm a real event lands in Turso `page_views` with the correct Origin-derived `site` (browsers send Origin on cross-origin sendBeacon; verified via curl on prompt-labs.org but not yet a real browser cross-origin). Cloudflare-proxied musicforge.app + recordings.pianohouseproject.org and the unclear domains (eaglerockventures, robotorchestra, ruhuman) still covered identically once instrumented — no longer deferred.
