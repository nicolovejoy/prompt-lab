...
### Claude routines — local-vs-cloud split (analysis 2026-06-03, not started)
Considered moving the nightly pipeline to Claude "routines" (cloud-run scheduled agents, run on Anthropic infra, laptop-independent, no permission prompts). Key constraint: routines have **no local-filesystem access**. The daily-summary step (`synthesizer.py` → `get_day_data()` reads raw `prompts`) is local-only — raw prompts live in `~/.claude/prompt-history.db` and are never synced to Turso by design, so that step **cannot** be a routine. Everything downstream reads only processed tables that are already on Turso, so it **is** routine-able: `send-review.py` (review email), `generate-report.py` (bi-monthly report), the synthesizer's weekly/intentions/snapshot steps, plus repo-hygiene agents (PR review, scheduled `/resync`). Routines do NOT fix the "LaunchAgent pauses when lid closed" pain — that's the laptop-bound daily step; fix is running the local synthesizer on the always-on mini. Caveat: routines run unattended with no permission gate (this session hit a classifier guard writing to prod Turso public tables — a routine would have none). Best first candidate: bi-monthly report as a routine that opens a PR (Turso-read only, repo-centric output, review gate). Would need `DB_BACKEND=turso` + secrets (`TURSO_*`, `ANTHROPIC_API_KEY`, Resend) injected as routine env vars.
