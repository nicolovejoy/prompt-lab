...
- `CONVENTIONS_AGENTS`: `behind` or `missing` → same calm fix, targeting `./AGENTS.md`. `tampered` → the same strong manual-review warning; do not suggest that `--apply` will repair it. `absent` → **do not treat as fine** — Codex discovers `AGENTS.md` automatically; without it, project conventions need an explicit read. One line offering `~/.claude/bin/sync-shared-md.sh --apply ./AGENTS.md`, plus: add a one-line preamble above the shared block ("Read CLAUDE.md in this repo first for project-specific conventions.") since the shared block alone won't point Codex at it. `in sync` / `skip` → silent. `unknown` → one line: sync-shared-md.sh printed nothing.
