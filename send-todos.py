...
# --- Scanner (duplicated from dashboard/server.py) ---
...
def _scan_todos():
    ...
    for claude_md in sorted(SRC_DIR.glob("*/CLAUDE.md")):
        ...
        for item in _parse_todo_sections(text):
            todos.append({"project": project, "section": item["section"],
                          "text": item["text"], "source": "CLAUDE.md"})
    for memory_md in sorted(CLAUDE_PROJECTS_DIR.glob("*/memory/MEMORY.md")):
        ...
        for item in _parse_todo_sections(text):
            todos.append({"project": project, "section": item["section"],
                          "text": item["text"], "source": "MEMORY.md"})
    return todos
